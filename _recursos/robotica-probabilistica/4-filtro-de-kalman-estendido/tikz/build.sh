#!/usr/bin/env bash
# Compila os diagramas TikZ da aula 4 (EKF) para SVG em ../images/
...
if [ "$#" -gt 0 ]; then
  ...
  FILES=(
    ekf-trajetoria-circular
    ekf-modelo-nao-linear
    ekf-gaussiana-distorcida
    ekf-taylor
    taylor-erro
    ekf-ciclo
    ekf-kf-comparacao
    ekf-qualidade-linearizacao
    ekf-mhekf
  )
fi
